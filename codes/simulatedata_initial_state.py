...
def simulatedata_with_initial_prob(b,m0,gamma_kbar,sig,kbar,prob,T):
    ...
    ##
    initial_state = np.unpackbits(np.array([np.argmax(prob)]).astype('uint8'))
    M_s[:,0] = 1-initial_state[-kbar:]
    ##
    dat = np.zeros(T)
